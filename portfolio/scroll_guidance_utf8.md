...
HereΓÇÖs how to create a basic parallax effect:
...
To add entry and exit effects to an element, you need to combine a few CSS properties. HereΓÇÖs a step-by-step guide:
...
- **DO** make sure the scope of the lookup works: When the element that is declaring the `view-timeline` is not a flat tree ancestor of the animated element, hoist up the visibility of the `view-timeline`ΓÇÖs name by using `timeline-scope` on a shared ancestor.
